 ...
 *
 *   <Lung days={smokeFreeDays} />        outline-only lungs; the stroke shifts
 *                                        from muted grey to violet as smoke-free
 *                                        days grow. No fill.
 *   <BurningCigarette days={smokingDays} /> a slim cigarette with a single muted
 *                                        ember and one smoke curl; the paper
 *                                        shortens and smoke lengthens as the
...
const EMBER = '#E06C5A'   // single restrained ember tone (no gradient)
...
export function Lung({ days = 0, size = 40 }) {
  ...
  return (
    <svg width={size} height={size} viewBox="0 0 48 48" fill="none" aria-label="lung health">
      ...
      <path d={LOBE} fill="none" stroke={line} strokeWidth="1.6" strokeLinejoin="round" />
  ...
  return (
    <svg width={size} height={size} viewBox="0 0 48 48" fill="none" aria-label="smoking streak">
      ...
      <rect x={filterX} y={yMid - h / 2} width="5" height={h} rx={h / 2}
        fill={active ? '#B9A98C' : '#7E8895'} fillOpacity={active ? 1 : 0.5} />
    </svg>
  )
}
